...
N = 60; % liczba próbek
use_window = false; % określa czy używać okna do poprawy transmitancji -> niwelowanie zafalowań

df = 1e-4;

% H = @(f) xpi_mod(f,0,0.1) + xpi_mod(f,1,0.1); % pożądana funkcja transmitancji
% H = @(f) xpi_mod(f,0.25,0.05) + xpi_mod(f,0.75,0.05); % pożądana funkcja transmitancji
H = @(f) xpi_mod(f,0.5,0.2); % pożądana funkcja transmitancji
...
subplot(2,2,3)
plot(0:N-1,real(h)); % hold on;
%plot(0:N-1,real(h2)); hold off;
title("real(h(n))")
xlabel("n")

subplot(2,2,4)
plot(0:N-1,imag(h));  %hold on;
% plot(0:N-1,imag(h2));hold off;
title("imag(h(n))")
xlabel("n")
